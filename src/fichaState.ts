...
/**
 * Aplica os campos do `clientProfile` aos campos correspondentes da Ficha,
 * preservando tudo o que o utilizador editou nas restantes secções.
 * Usado quando o perfil é editado depois da Ficha já ter sido aberta.
 */
export function applyProfileToFicha(profile: ClientProfile, current: FichaState): FichaState {
  return {
    ...current,
    identificacao: {
      ...current.identificacao,
      nome: profile.nomeCliente || current.identificacao.nome,
      nif: profile.nif || current.identificacao.nif,
      estadoCivil: profile.estadoCivil || current.identificacao.estadoCivil,
      dependentes: profile.nrDependentes ?? current.identificacao.dependentes,
      morada: [profile.morada, profile.codigoPostal, profile.localidade].filter(Boolean).join(', ') || current.identificacao.morada,
      telefone: profile.telefone || current.identificacao.telefone,
      email: profile.email || current.identificacao.email,
    },
    situacaoAtual: {
      ...current.situacaoAtual,
      tipo: tipoEntidadeMap[profile.tipoEntidade] || current.situacaoAtual.tipo,
    },
    atividade: {
      ...current.atividade,
      caePrincipal: profile.cae || current.atividade.caePrincipal,
    },
    faturacao: {
      ...current.faturacao,
      ano1: profile.faturaçaoAnualPrevista || current.faturacao.ano1,
    },
    rh: {
      ...current.rh,
      numero: profile.nrFuncionarios ?? current.rh.numero,
    },
  };
}
