...
type Regime = 'geral' | 'madeira' | 'acores' | 'interioridade' | 'startup';
type Territorio = 'continental' | 'madeira' | 'acores';
type FuelType = 'convencional' | 'plug_in' | 'plug_in_5050' | 'gnv' | 'eletrico';

interface ViaturaRow {
  id: string;
  ano: number;
...
export interface PreviSaState {
  ...
  // Q07 — Apuramento
  c701_rai: number; // Resultado antes de impostos
  c702: number; // Variações de justo valor (acrescer)
  c703: number; // Ajustamentos não dedutíveis
  c704: number; // Variações de justo valor (deduzir)
  c705: number; // Reversão de ajustamentos
  c706: number; // Mais-valias contabilísticas
  c707: number; // Menos-valias contabilísticas
  ...
  prejuizosDeduzir: number; // Prejuízos fiscais anteriores a deduzir (limite 65%)
  limiteMaisPP: boolean; // Aumentar limite de dedução em +10pp
  beneficiosFiscais: number; // Outros benefícios fiscais (Q07 campo 774+775+...)
  ...
  ta_bonus: number;
  agravamentoTA: boolean; // empresa com prejuízo fiscal
  // Q10 — PEC / PC
...
export const defaultPreviSaState = (): PreviSaState => ({
...
export default function PreviSaSimulator() {
  const [state, setState] = useState<PreviSaState>(defaultPreviSaState);
