...
export default function ImoveisEmpresa({ initialState, onStateChange, profile }: Props) {
  ...
  const leftCls = { split: "bg-white border-b border-[#E2E8F0] xl:border-b-0 xl:border-r xl:overflow-y-auto p-4 sm:p-5 xl:p-[28px] flex flex-col gap-4 xl:gap-[22px] xl:h-full", stacked: "bg-white border-b-2 border-[#E2E8F0] p-6 flex flex-col gap-5", mosaic: "bg-white rounded-[20px] border border-emerald-100 shadow-sm overflow-y-auto p-5 flex flex-col gap-5 h-full", compact: "max-w-xl mx-auto p-4 pb-0 w-full", hero: "md:w-[400px] shrink-0 bg-white border-l border-[#E2E8F0] overflow-y-auto p-6 flex flex-col gap-5 h-full" }[simMode];
  const rightCls = { split: "p-4 sm:p-5 xl:p-[28px] xl:overflow-y-auto xl:h-full flex flex-col gap-4 xl:gap-[16px]", stacked: "p-6 flex flex-col gap-4 max-w-7xl mx-auto w-full", mosaic: "bg-white rounded-[20px] border border-emerald-100 shadow-sm overflow-y-auto p-5 flex flex-col gap-4 h-full", compact: "max-w-xl mx-auto p-4 pt-2 w-full border-t border-slate-100", hero: "flex-1 overflow-y-auto p-6 flex flex-col gap-4" }[simMode];
  ...
  const steps: FlowStep<ImoveisState>[] = [
    {
      id: 'valor_tipo',
      label: 'Valor e Tipo do Imóvel',
      description: 'Indique o valor do imóvel e o tipo de uso pretendido.',
      render: (st, setSt) => (
        <div className="space-y-[18px]">
          ...
          <div>
            ...
            <select value={st.tipoUso} onChange={e => setSt({ tipoUso: e.target.value as ImoveisState['tipoUso'] })} className={inputCls}>
    ...
    {
      id: 'horizonte',
      label: 'Horizonte e Atividade',
      description: 'Escolha o horizonte de investimento e o setor de atividade.',
      render: (st, setSt) => (
        <div className="space-y-[18px]">
          <div>
            <label className={labelCls}>Horizonte de Investimento <Tip>Quanto tempo prevê manter este imóvel: curto prazo (menos de 5 anos) ou longo prazo (mais de 5 anos).</Tip></label>
            <select value={st.horizonteInvestimento} onChange={e => setSt({ horizonteInvestimento: e.target.value as ImoveisState['horizonteInvestimento'] })} className={inputCls}>
              <option value="curto">Curto prazo (&lt;5 anos)</option>
              <option value="longo">Longo prazo (&gt;5 anos)</option>
            </select>
          </div>
          <div>
            <label className={labelCls}>Setor de Atividade <Tip>O setor principal da empresa. Afeta o tratamento fiscal do imóvel.</Tip></label>
            <select value={st.tipoAtividade} onChange={e => setSt({ tipoAtividade: e.target.value as ImoveisState['tipoAtividade'] })} className={inputCls}>
    ...
    },
    {
      ...
      description: 'Indique as preferências e restrições relevantes para a decisão.',
      render: (st, setSt) => (
        <div className="space-y-[18px]">
          {[
            { key: 'precisaLiquidezMensal', label: 'Precisa de liquidez mensal das rendas', tip: 'Se precisa de receber rendas mensalmente para cobrir despesas pessoais ou da empresa.' },
  ...
  const resultsContent = (
    <>
      ...
      {analise && (
        <>
          <div className={cn("rounded-[16px] p-[16px] border-2 flex items-center gap-3", analise.recomendado === 'arrendamento' ? "bg-blue-50 border-blue-300" : analise.recomendado === 'especies' ? "bg-emerald-50 border-emerald-300" : "bg-amber-50 border-amber-300")}>
            <ArrowRight className={cn("w-5 h-5 shrink-0", analise.recomendado === 'arrendamento' ? "text-blue-600" : analise.recomendado === 'especies' ? "text-emerald-600" : "text-amber-600")} />
            <div>
              <div className={cn("text-[13px] font-[800]", analise.recomendado === 'arrendamento' ? "text-blue-800" : analise.recomendado === 'especies' ? "text-emerald-800" : "text-amber-800")}>
                {analise.recomendado === 'arrendamento' ? 'Recomendado: Arrendamento / Comodato' : analise.recomendado === 'especies' ? 'Recomendado: Entrada em Espécie' : 'Análise neutra — consulte o contabilista'}
              </div>
          ...
          <div className="grid grid-cols-2 gap-[12px]">
            {/* Arrendamento */}
            <div className={cn("bg-white border-2 rounded-[20px] p-[18px]", analise.recomendado === 'arrendamento' ? "border-blue-300" : "border-[#E2E8F0]")}>
              <div className="flex items-center gap-2 mb-[14px]">
                <div className="w-7 h-7 bg-blue-100 rounded-[8px] flex items-center justify-center"><Home className="w-4 h-4 text-blue-600" /></div>
                <span className="text-[13px] font-[800] text-[#0F172A]">Arrendamento / Comodato</span>
                {analise.recomendado === 'arrendamento' && <span className="ml-auto text-[10px] bg-blue-600 text-white px-2 py-0.5 rounded-full font-[700]">✓ Recomendado</span>}
              </div>
              <div className="space-y-[6px] text-[12px]">
                <div className="flex justify-between"><span className="text-[#64748B]">Custo inicial</span><span className="font-[700] text-emerald-700">0 €</span></div>
                <div className="flex justify-between"><span className="text-[#64748B]">Renda anual est.</span><span className="font-[700]">{ptEur(analise.rendaAnualEstimada)}</span></div>
                <div className="flex justify-between"><span className="text-[#64748B]">IRS/IRC sobre rendas</span><span className="font-[700] text-red-600">~ {ptEur(analise.irsRendas)}</span></div>
                <div className="flex justify-between"><span className="text-[#64748B]">Reforço balanço</span><span className="font-[700] text-[#94A3B8]">Não</span></div>
              </div>
              <div className="mt-[12px] space-y-[5px]">
                {pros_arrendamento.map((p, i) => (
                  <div key={i} className="flex items-start gap-2"><CheckCircle className="w-3.5 h-3.5 text-emerald-500 shrink-0 mt-[2px]" /><span className="text-[11px] text-[#475569]">{p}</span></div>
                ))}
                {contras_arrendamento.map((c, i) => (
                  <div key={i} className="flex items-start gap-2"><XCircle className="w-3.5 h-3.5 text-red-400 shrink-0 mt-[2px]" /><span className="text-[11px] text-[#94A3B8]">{c}</span></div>
                ))}
              </div>
            </div>
            {/* Entrada em Espécie */}
            <div className={cn("bg-white border-2 rounded-[20px] p-[18px]", analise.recomendado === 'especies' ? "border-emerald-300" : "border-[#E2E8F0]")}>
              <div className="flex items-center gap-2 mb-[14px]">
                <div className="w-7 h-7 bg-emerald-100 rounded-[8px] flex items-center justify-center"><Building className="w-4 h-4 text-emerald-600" /></div>
                <span className="text-[13px] font-[800] text-[#0F172A]">Entrada em Espécie</span>
                {analise.recomendado === 'especies' && <span className="ml-auto text-[10px] bg-emerald-600 text-white px-2 py-0.5 rounded-full font-[700]">✓ Recomendado</span>}
              </div>
              <div className="space-y-[6px] text-[12px]">
                <div className="flex justify-between"><span className="text-[#64748B]">IMT estimado</span><span className="font-[700] text-red-600">{ptEur(analise.imt)}</span></div>
                <div className="flex justify-between"><span className="text-[#64748B]">Imposto de Selo (0,8%)</span><span className="font-[700] text-red-600">{ptEur(analise.impostoSelo)}</span></div>
                <div className="flex justify-between"><span className="text-[#64748B]">Custo inicial total</span><span className="font-[700] text-red-600">{ptEur(analise.custoInicialEspecies)}</span></div>
                <div className="flex justify-between"><span className="text-[#64748B]">Reforço balanço</span><span className="font-[700] text-emerald-700">{ptEur(s.valorImovel)}</span></div>
                <div className="flex justify-between"><span className="text-[#64748B]">Depreciação fiscal/ano</span><span className="font-[700] text-emerald-700">{ptEur(analise.depreciacaoAnual)}</span></div>
              </div>
              <div className="mt-[12px] space-y-[5px]">
          ...
          {s.temApoiosPT2030 && (
            <div className="bg-amber-50 border border-amber-200 rounded-[16px] p-[14px] flex items-start gap-2">
              ...
              <p className="text-[13px] text-amber-800 font-[500]">Com apoios PT2030 ativos, a entrada em espécie pode estar sujeita a restrições de transmissão patrimonial.</p>
            </div>
          )}
