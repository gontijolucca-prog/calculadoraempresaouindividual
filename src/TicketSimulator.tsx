...
const LEGAL_NOTES: Record<TipoTicket, { limitsNote: string; ircNote: string; refs: string }> = {
  ...
  restaurante: {
    limitsNote: 'Limite diário isento: €10,46 (cartão eletrónico) · €6,15 (dinheiro/transferência)',
...
export default function TicketSimulator({ initialState, onStateChange }: Props) {
  ...
  const outerCls = { split: "overflow-y-auto xl:overflow-hidden xl:h-full xl:grid xl:grid-cols-[420px_1fr] bg-[#F5F7FA] text-[#1E293B]", stacked: "h-full flex flex-col bg-[#F0F4F8] text-[#1E293B] overflow-y-auto", mosaic: "h-full bg-[#F0FDF4] text-[#1E293B] md:grid md:grid-cols-2 gap-4 p-4", compact: "h-full overflow-y-auto bg-white text-[#1E293B]", hero: "h-full flex md:flex-row-reverse overflow-hidden bg-[#F5F5F4] text-[#1E293B]" }[simMode];
  const leftCls = { split: "bg-white border-b border-[#E2E8F0] xl:border-b-0 xl:border-r xl:overflow-y-auto p-4 sm:p-5 xl:p-[28px] flex flex-col gap-4 xl:h-full", stacked: "bg-white border-b-2 border-[#E2E8F0] p-6 flex flex-col gap-5", mosaic: "bg-white rounded-[20px] border border-emerald-100 shadow-sm overflow-y-auto p-5 flex flex-col gap-5 h-full", compact: "max-w-xl mx-auto p-4 pb-0 w-full", hero: "md:w-[440px] shrink-0 bg-white border-l border-[#E2E8F0] overflow-y-auto p-6 flex flex-col gap-5 h-full" }[simMode];
  const rightCls = { split: "p-4 sm:p-5 xl:p-[28px] xl:overflow-y-auto xl:h-full flex flex-col gap-4", stacked: "p-6 flex flex-col gap-4 max-w-7xl mx-auto w-full", mosaic: "bg-white rounded-[20px] border border-emerald-100 shadow-sm overflow-y-auto p-5 flex flex-col gap-4 h-full", compact: "max-w-xl mx-auto p-4 pt-2 w-full border-t border-slate-100", hero: "flex-1 overflow-y-auto p-6 flex flex-col gap-4" }[simMode];
    ...
    <>
      {!calc ? (
        ...
        <>
          ...
          {isCar && (
            <div className="bg-blue-50 border border-blue-200 rounded-[16px] p-5 flex items-start gap-3">
              <Fuel className="w-5 h-5 text-blue-600 shrink-0 mt-0.5" />
              <div>
                <p className="text-[13px] font-[700] text-blue-900 mb-1">Ticket Car® — gestão de frota</p>
                <p className="text-[12px] text-blue-800 font-[500] leading-relaxed">
                  Não tem isenção de IRS nem de SS — é uma ferramenta de controlo de despesas com viaturas. O benefício fiscal é a recuperação de IVA em combustível e manutenção (CIVA Art. 21.º) e a dedutibilidade a 100% em IRC. Tributação Autónoma pode aplicar-se (CIRC Art. 88.º).
                </p>
              </div>
            </div>
          )}
          ...
          {isCar && (
            <div className="bg-[#0F172A] text-white rounded-[20px] p-6">
              <div className="text-[11px] font-[700] uppercase tracking-[1px] text-slate-400">
                IVA Recuperável (Frota) · {(calc.ivaRate * 100).toFixed(0)}% de dedução
              </div>
              <div className="text-[48px] font-[900] leading-none mt-2 tracking-[-2px]">
                {ptEur(calc.ivaRecuperavel)}
              </div>
              <div className="text-[13px] text-slate-400 mt-2">
                {s.employees} viatura{s.employees !== 1 ? 's' : ''} · custo total {ptEur(calc.custoAnual)}
              </div>
              {calc.ivaRate === 0 && (
                <div className="text-[12px] text-amber-400 font-[600] mt-2">
                  Ligeiros de passageiros: IVA excluído (CIVA Art. 21.º n.º 1 a)) — sem recuperação de IVA
                </div>
              )}
            </div>
          )}
          ...
          <div className="bg-white border border-[#E2E8F0] rounded-[20px] p-5">
            ...
            <div className="space-y-3 text-[14px]">
              ...
              <div className={cn(
                ...
                isCar ? "text-blue-700 bg-blue-50 border border-blue-100" :
                ...
                {isCar && "100% dedutível como gasto com viaturas (CIRC Art. 23.º). Atenção: Tributação Autónoma pode aplicar-se sobre despesas de viaturas (CIRC Art. 88.º)."}
              </div>
            </div>
          </div>
          ...
          {isCar && (
            <div className="bg-white border border-[#E2E8F0] rounded-[20px] p-5">
              <div className="flex items-center gap-3 mb-4">
                ...
                <h3 className="text-[14px] font-[700] text-[#0F172A]">Resumo Fiscal da Frota</h3>
              </div>
              <div className="space-y-3 text-[14px]">
                <div className="flex justify-between">
                  <span className="text-[#64748B]">Custo total da frota (anual)</span>
                  <span className="font-[700]">{ptEur(calc.custoAnual)}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-[#64748B]">Dedutível em IRC (100%)</span>
                  <span className="font-[700] text-blue-600">{ptEur(calc.dedutivelIRC)}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-[#64748B]">IVA recuperável ({(calc.ivaRate * 100).toFixed(0)}% · CIVA Art. 21.º)</span>
                  <span className="font-[700] text-emerald-600">{ptEur(calc.ivaRecuperavel)}</span>
                </div>
                <div className="h-px bg-[#F1F5F9]" />
                <div className="flex justify-between text-[15px]">
                  <span className="font-[800] text-[#0F172A]">Custo líquido estimado</span>
                  <span className="font-[900] text-[#0F172A]">{ptEur(calc.custoAnual - calc.ivaRecuperavel)}</span>
                </div>
                <div className="text-[11px] text-[#94A3B8] leading-relaxed">
                  Custo líquido = custo bruto − IVA recuperável. Benefício IRC reflecte-se na liquidação anual. Tributação Autónoma calculada separadamente consoante o tipo e ano de viatura.
                </div>
              </div>
            </div>
          )}
  ...
  const steps: FlowStep<TicketSimulatorState>[] = [
    {
      id: 'tipo',
      label: 'Tipo de Ticket e Beneficiários',
      description: 'Escolha o tipo de ticket e indique quantos beneficiários (funcionários, viaturas ou destinatários).',
      render: () => (
    ...
    },
    {
      ...
      description: isRestaurante
        ? 'Defina a modalidade de pagamento, valor diário, dias úteis por mês e meses por ano.'
        : isCar
          ? 'Indique o tipo de viatura e o custo anual por viatura.'
          : 'Indique o valor anual atribuído a cada beneficiário.',
      render: () => (
        <div className="flex flex-col gap-4">
          {isRestaurante && (
            <>
