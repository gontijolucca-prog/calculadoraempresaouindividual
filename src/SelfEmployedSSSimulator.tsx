...
export default function SelfEmployedSSSimulator({ initialState, onStateChange }: Props) {
  ...
  const outerCls = { split: "overflow-y-auto xl:overflow-hidden xl:h-full xl:grid xl:grid-cols-[400px_1fr] bg-[#F5F7FA] text-[#1E293B]", stacked: "h-full flex flex-col bg-[#F0F4F8] text-[#1E293B] overflow-y-auto", mosaic: "h-full bg-[#F0FDF4] text-[#1E293B] md:grid md:grid-cols-2 gap-4 p-4", compact: "h-full overflow-y-auto bg-white text-[#1E293B]", hero: "h-full flex md:flex-row-reverse overflow-hidden bg-[#F5F5F4] text-[#1E293B]" }[simMode];
  const leftCls = { split: "bg-white border-b border-[#E2E8F0] xl:border-b-0 xl:border-r xl:overflow-y-auto p-4 sm:p-6 xl:p-[40px] flex flex-col gap-5 xl:gap-[32px] xl:h-full", stacked: "bg-white border-b-2 border-[#E2E8F0] p-6 flex flex-col gap-6", mosaic: "bg-white rounded-[20px] border border-emerald-100 shadow-sm overflow-y-auto p-5 flex flex-col gap-5 h-full", compact: "max-w-xl mx-auto p-4 pb-0 w-full", hero: "md:w-[420px] shrink-0 bg-white border-l border-[#E2E8F0] overflow-y-auto p-6 flex flex-col gap-5 h-full" }[simMode];
  const rightCls = { split: "p-4 sm:p-6 xl:p-[40px] xl:overflow-y-auto xl:h-full max-w-7xl mx-auto w-full flex flex-col gap-5 xl:gap-[32px]", stacked: "p-6 flex flex-col gap-6 max-w-7xl mx-auto w-full", mosaic: "bg-white rounded-[20px] border border-emerald-100 shadow-sm overflow-y-auto p-5 flex flex-col gap-5 h-full", compact: "max-w-xl mx-auto p-4 pt-2 w-full border-t border-slate-100", hero: "flex-1 p-6 md:p-[40px] overflow-y-auto flex flex-col gap-5" }[simMode];
  ...
  const steps: FlowStep<SSState>[] = [
    ...
    },
    {
      ...
      description: 'Escolha se presta serviços ou vende bens/produtos.',
      render: (st, setSt) => (
        <div>
          ...
          <select value={st.tipoRendimento} onChange={e => setSt({ tipoRendimento: e.target.value as 'servicos' | 'bens' })} className={inputClass}>
    ...
    },
    {
      ...
      description: 'Novos trabalhadores independentes têm isenção no 1.º ano.',
      render: (st, setSt) => (
        <label className={cn(
          "flex items-start gap-4 p-5 border-2 rounded-[16px] cursor-pointer transition-colors",
          st.primeiroAno ? "border-emerald-400 bg-emerald-50" : "border-[#E2E8F0] hover:border-[#CBD5E1]"
        )}>
          <input
            type="checkbox"
            checked={st.primeiroAno}
            onChange={e => setSt({ primeiroAno: e.target.checked })}
      ...
      {!primeiroAno && result && (
        <>
          ...
          <div className="grid grid-cols-1 xl:grid-cols-4 gap-[20px]">
            <div className="bg-emerald-50 border-2 border-emerald-300 rounded-[24px] p-[24px] md:p-[28px] flex flex-col shadow-sm ring-4 ring-emerald-100">
              <div className="flex items-center gap-[12px] mb-[16px]">
                <div className="bg-emerald-100 text-emerald-600 p-[10px] rounded-[14px]">
                  <Wallet className="w-[20px] h-[20px]" />
                </div>
                <h3 className="text-[15px] font-[700] text-[#0F172A]">Líquido Mensal <Tip>O rendimento que sobra depois de pagar a contribuição mensal à Segurança Social.</Tip></h3>
              </div>
              <div className="text-[36px] font-[800] text-emerald-700 tracking-[-1px] mb-[8px]">
                {ptEur(income - result.mensal)}
              </div>
              <p className="text-[12px] text-[#64748B] font-[500]">{ptEur(income)} − {ptEur(result.mensal)}</p>
            </div>
            ...
            <div className="bg-white border border-[#E2E8F0] rounded-[24px] p-[24px] md:p-[28px] flex flex-col shadow-sm">
              <div className="flex items-center gap-[12px] mb-[16px]">
                ...
                <h3 className="text-[15px] font-[700] text-[#0F172A]">SS Mensal <Tip>Estimativa mensal de SS. Na prática a SS é paga trimestralmente, mas este valor dá a ideia do encargo por mês.</Tip></h3>
              </div>
          ...
          <div className="bg-white border border-[#E2E8F0] rounded-[24px] p-[24px] md:p-[32px] flex flex-col shadow-sm">
            ...
            <div className="flex-1 space-y-0">
              ...
              <div className="flex justify-between py-[14px] border-t-2 border-emerald-100 text-[14px] items-center bg-emerald-50/50 rounded-b-[8px] px-2 -mx-2">
                <span className="text-emerald-900 font-[700]">Rendimento Líquido Mensal <Tip>O que sobra no bolso depois de pagar a contribuição mensal à Segurança Social.</Tip></span>
                <span className="font-[800] font-mono text-emerald-700">{ptEur(income - result.mensal)}</span>
              </div>
              <div className="flex justify-between py-[14px] border-t border-emerald-100 text-[14px] items-center bg-emerald-50/30 rounded-b-[8px] px-2 -mx-2">
                <span className="text-emerald-800 font-[600]">Rendimento Líquido Anual <Tip>O total anual que sobra depois de pagar SS durante todo o ano.</Tip></span>
                <span className="font-[700] font-mono text-emerald-600">{ptEur(income * 12 - result.anual)}</span>
              </div>
            </div>
