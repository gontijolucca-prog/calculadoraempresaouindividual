      ...
      <div className={leftCls}>
        ...
        <section className="space-y-[14px]">
          ...
          <div className="grid grid-cols-2 gap-[10px]">
            <div>
              <label className={labelCls}>Capitais Próprios (€)</label>
              <input type="number" min="0" value={d.capitaisProprios || ''} onChange={e => setState({ capitaisProprios: parseFloat(e.target.value) || 0 })} className={inputCls} placeholder="0" />
            </div>
            <div>
              <label className={labelCls}>Ativo Total (€)</label>
              <input type="number" min="0" value={d.ativoTotal || ''} onChange={e => setState({ ativoTotal: parseFloat(e.target.value) || 0 })} className={inputCls} placeholder="0" />
            </div>
            <div className="col-span-2">
              <label className={labelCls}>Passivo Total (€)</label>
              <input type="number" min="0" value={d.passivoTotal || ''} onChange={e => setState({ passivoTotal: parseFloat(e.target.value) || 0 })} className={inputCls} placeholder="0" />
            </div>
          </div>
        </section>
        ...
        <section className="space-y-[14px]">
          ...
          <div className="grid grid-cols-2 gap-[10px]">
            <div>
              <label className={labelCls}>Ativo Corrente (€)</label>
              <input type="number" min="0" value={d.ativoCorrente || ''} onChange={e => setState({ ativoCorrente: parseFloat(e.target.value) || 0 })} className={inputCls} placeholder="0" />
            </div>
            <div>
              <label className={labelCls}>Passivo Corrente (€)</label>
              <input type="number" min="0" value={d.passivoCorrente || ''} onChange={e => setState({ passivoCorrente: parseFloat(e.target.value) || 0 })} className={inputCls} placeholder="0" />
            </div>
            <div>
              <label className={labelCls}>Disponibilidades (€)</label>
              <input type="number" min="0" value={d.disponibilidades || ''} onChange={e => setState({ disponibilidades: parseFloat(e.target.value) || 0 })} className={inputCls} placeholder="0" />
            </div>
            <div>
              <label className={labelCls}>Custo Fixo Mensal (€)</label>
              <input type="number" min="0" value={d.custoFixoMensal || ''} onChange={e => setState({ custoFixoMensal: parseFloat(e.target.value) || 0 })} className={inputCls} placeholder="0" />
            </div>
          </div>
        </section>
        ...
        <section className="space-y-[14px]">
          ...
          <div className="grid grid-cols-2 gap-[10px]">
            <div>
              <label className={labelCls}>Resultado Líquido (€/ano)</label>
              <input type="number" value={d.resultadoLiquido || ''} onChange={e => setState({ resultadoLiquido: parseFloat(e.target.value) || 0 })} className={inputCls} placeholder="0" />
            </div>
            <div>
              <label className={labelCls}>Volume de Negócios (€/ano)</label>
              <input type="number" min="0" value={d.volumeNegocios || ''} onChange={e => setState({ volumeNegocios: parseFloat(e.target.value) || 0 })} className={inputCls} placeholder="0" />
            </div>
          </div>
          <div>
        ...
        <section className="space-y-[14px]">
          ...
          <div className="grid grid-cols-2 gap-[10px]">
            <div>
              <label className={labelCls}>Faturação maior cliente (€/ano)</label>
              <input type="number" min="0" value={d.faturacaoMaiorCliente || ''} onChange={e => setState({ faturacaoMaiorCliente: parseFloat(e.target.value) || 0 })} className={inputCls} placeholder="0" />
            </div>
            <div>
              <label className={labelCls}>Financiamento externo (€)</label>
              <input type="number" min="0" value={d.financiamentoExterno || ''} onChange={e => setState({ financiamentoExterno: parseFloat(e.target.value) || 0 })} className={inputCls} placeholder="0" />
            </div>
            <div className="col-span-2">
              <label className={labelCls}>Total de fontes de financiamento (€)</label>
              <input type="number" min="0" value={d.totalFinanciamento || ''} onChange={e => setState({ totalFinanciamento: parseFloat(e.target.value) || 0 })} className={inputCls} placeholder="0" />
            </div>
          </div>
        </section>
        ...
        <section className="space-y-[14px]">
          ...
          <div className="space-y-[8px]">
            {[
              { key: 'processosDefinidos', label: 'Processos e procedimentos documentados' },
              { key: 'softwareGestao', label: 'Software de gestão/ERP implementado' },
              { key: 'equipaAutonoma', label: 'Equipa capaz de operar sem o gerente' },
              { key: 'baixaDependenciaGerente', label: 'Baixa dependência pessoal do gerente/sócio' },
              { key: 'controlFinanceiro', label: 'Controlo financeiro e reporting mensal' },
            ].map(({ key, label }) => (
              <label key={key} className="flex items-center gap-3 cursor-pointer group">
                ...
                <span className="text-[13px] text-[#475569] font-[500] group-hover:text-[#0F172A] transition-colors">{label}</span>
              </label>
            ))}
          </div>
        </section>
