...
function useVehicleResults(state: VehicleSimulatorState) {
  // Motor puro em src/lib/viaturas.ts (testável).
  return useMemo(() => calcViatura(state), [state]);
}
        ...
        <div className="space-y-[24px]">
          ...
          {category === 'passageiros' && !exemptTA && (
            <label className="flex items-start gap-4 p-5 border-2 border-[#E2E8F0] rounded-[16px] cursor-pointer transition-colors hover:border-[#CBD5E1]">
              ...
              <div>
                <span className="text-[14px] font-[700] text-[#0F172A] block">Empresa com prejuízo fiscal? <Tip>Se a empresa apresentar prejuízo fiscal no período, as taxas de Tributação Autónoma sobem +10 pontos percentuais (CIRC art.88 n.14).</Tip></span>
                <span className="text-[12px] text-[#64748B] font-[500] leading-snug mt-1 block">Agrava a Tributação Autónoma em +10 p.p.</span>
              </div>
            </label>
          )}
        </div>
