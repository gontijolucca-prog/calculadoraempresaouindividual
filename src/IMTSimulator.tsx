...
export default function IMTSimulator({ initialState, onStateChange }: Props) {
  ...
  const steps: FlowStep<IMTState>[] = [
    {
      id: 'valor_tipo',
      label: 'Valor e Tipo de Imóvel',
      description: 'Indique o preço de compra e a categoria do imóvel.',
      render: (st, setSt) => (
        <div className="space-y-[20px]">
          ...
          <div>
            <label className={labelCls}>Tipo de Imóvel <Tip>HPP é Habitação Própria Permanente (onde vai viver). Habitação secundária é uma segunda casa.</Tip></label>
            <select value={st.tipo} onChange={e => setSt({ tipo: e.target.value as TipoImovel })} className={inputCls}>
    ...
    {
      id: 'localizacao_hpp',
      label: 'Localização e Primeira Habitação',
      description: 'Onde fica o imóvel e se é a sua primeira habitação própria.',
      render: (st, setSt) => (
        <div className="space-y-[20px]">
          <div>
            <label className={labelCls}>Localização <Tip>Nos Açores e Madeira os escalões do IMT são 25% mais elevados.</Tip></label>
            <div className="relative">
              <MapPin className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-[#64748B]" />
              <select value={st.localizacao} onChange={e => setSt({ localizacao: e.target.value as Localizacao })} className={cn(inputCls, "pl-9")}>
          ...
          {(st.tipo === 'hpp' || st.tipo === 'habitacao') && (
            <label className={cn("flex items-center gap-3 p-[14px] rounded-[12px] border-2 cursor-pointer transition-colors", st.primeiraHabitacao ? "bg-[#0F172A]/5 border-[#0F172A]" : "bg-[#F5F7FA] border-[#E2E8F0] hover:border-[#94A3B8]")}>
              ...
              <span className="text-[13px] font-[600] text-[#475569]">Primeira habitação (HPP) <Tip>Se é a primeira vez que compra uma casa para habitação própria permanente.</Tip></span>
            </label>
          )}
        </div>
      ),
    },
    {
      ...
      description: 'A idade no momento da escritura. Até 35 anos pode beneficiar do IMT Jovem.',
      render: (st, setSt) => (
        <div>
  ...
  const resultsContent = (
    <>
      ...
      {result && (
        <>
          {result.isentoJovem && (
            <div className={cn("rounded-[16px] p-[16px] border-2 flex items-center gap-3", result.isento ? "bg-emerald-50 border-emerald-300" : "bg-amber-50 border-amber-300")}>
              <CheckCircle className={cn("w-5 h-5 shrink-0", result.isento ? "text-emerald-600" : "text-amber-600")} />
              <div>
                <div className={cn("text-[13px] font-[800]", result.isento ? "text-emerald-800" : "text-amber-800")}>{result.isento ? 'Isento de IMT e Imposto de Selo' : 'IMT Jovem — Redução parcial'}</div>
                <div className={cn("text-[12px] mt-[2px]", result.isento ? "text-emerald-700" : "text-amber-700")}>CIMT Art. 11º-A — OE 2026</div>
              </div>
            </div>
          )}
          <div className="grid grid-cols-3 gap-[12px]">
            {[
              { label: 'IMT', value: result.imt, color: result.imt === 0 ? 'emerald' : 'red', tip: 'Imposto Municipal sobre Transmissões: calculado em escalões progressivos sobre o preço de compra.' },
              { label: 'Imposto de Selo', value: result.impostoSelo, color: result.impostoSelo === 0 ? 'emerald' : 'orange', tip: 'Imposto de Selo de 0,8% sobre o valor de transação (TGIS verba 1.1).' },
              { label: 'Total Impostos', value: result.total, color: result.total === 0 ? 'emerald' : 'red', tip: 'Soma do IMT + Imposto de Selo. É o custo fiscal total da compra.' },
            ].map(({ label, value, color, tip }) => (
              <div key={label} className={cn("rounded-[16px] p-[16px] border text-center", color === 'emerald' ? 'bg-emerald-50 border-emerald-100' : color === 'orange' ? 'bg-orange-50 border-orange-100' : 'bg-red-50 border-red-100')}>
                <div className="text-[11px] font-[700] uppercase tracking-[1px] text-[#64748B]">{label} <Tip>{tip}</Tip></div>
                <div className={cn("text-[22px] font-[900] mt-[4px]", color === 'emerald' ? 'text-emerald-700' : color === 'orange' ? 'text-orange-700' : 'text-red-700')}>{ptEur(value)}</div>
                {label === 'IMT' && value > 0 && (<div className="text-[11px] text-[#94A3B8] mt-1">taxa {pct(result.taxaAplicada)}</div>)}
              </div>
            ))}
          </div>
          <div className="bg-white border border-[#E2E8F0] rounded-[20px] p-[20px]">
            ...
            <div className="space-y-[10px]">
              <div className="flex justify-between text-[14px]"><span className="text-[#64748B]">Valor de aquisição</span><span className="font-[700] text-[#0F172A]">{ptEur(s.valor)}</span></div>
              <div className="flex justify-between text-[14px]"><span className="text-[#64748B]">IMT</span><span className={cn("font-[700]", result.imt === 0 ? "text-emerald-600" : "text-red-600")}>{result.imt === 0 ? 'Isento' : `- ${ptEur(result.imt)}`}</span></div>
              <div className="flex justify-between text-[14px]"><span className="text-[#64748B]">Imposto de Selo (0,8%)</span><span className={cn("font-[700]", result.impostoSelo === 0 ? "text-emerald-600" : "text-orange-600")}>{result.impostoSelo === 0 ? 'Isento' : `- ${ptEur(result.impostoSelo)}`}</span></div>
              <div className="h-px bg-[#E2E8F0] my-2" />
              <div className="flex justify-between text-[15px]"><span className="font-[700] text-[#0F172A]">Valor total de aquisição</span><span className="font-[900] text-[#0F172A]">{ptEur(s.valor + result.total)}</span></div>
