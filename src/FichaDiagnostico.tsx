...
function Check({ label, checked, onChange, tip }: { label: string; checked: boolean; onChange: (v: boolean) => void; tip?: string }) {
  return (
    <label className="flex items-center gap-2 cursor-pointer py-1 px-2 rounded-[6px] hover:bg-slate-50 transition-colors">
      ...
      />
      <span className="text-[13px] font-[600] text-[#334155]">
        {label}
        {tip && <> <Tip>{tip}</Tip></>}
      </span>
    </label>
...
export default function FichaDiagnostico({ initialState, onStateChange, openLegalAt, clientProfile }: Props) {
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.35, ease: [0.32, 0.72, 0, 1] }} className="h-full bg-[#F8FAFC] overflow-y-auto" id="ficha-root">
      {/* Header */}
          ...
          <div className="min-w-0 max-w-4xl space-y-8">
            ...
            <Section id="fd-atividade" icon={FileText} title="3. Tipo de Atividade" legalAnchor="legal-cae" openLegalAt={openLegalAt}>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <Field label="CAE principal" tip="Classificação Portuguesa das Atividades Económicas — código de 5 dígitos atribuído pelo INE que identifica o setor da empresa nas Finanças e Segurança Social.">
                ...
                <Field label="CAE secundários (se existirem)" tip="Atividades adicionais à principal, separadas por vírgulas. Relevante para enquadramento IVA quando há mistura de regimes.">
            ...
            <Section id="fd-iva" icon={Calculator} title="5. Regime de IVA" color="#1D4ED8" legalAnchor="legal-iva" openLegalAt={openLegalAt}>
              <div className="space-y-4">
                ...
                <div>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-1">
                    <Check label="Artigo 53.º (isenção PME)" tip="Regime de isenção de IVA para PMEs com faturação anual ≤ €15.000 (CIVA Art. 53.º). Não cobra IVA aos clientes mas também não deduz IVA suportado." checked={s.iva.enquadramento.art53} onChange={v => update('iva', { ...s.iva, enquadramento: { ...s.iva.enquadramento, art53: v } })} />
            ...
            <Section id="fd-documentos" icon={FolderCheck} title="16. Documentos a Recolher" legalAnchor="legal-docs" openLegalAt={openLegalAt}>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-1">
                ...
                <Check label="Balancete atual (se empresa)" tip="Documento contabilístico que mostra os saldos das contas no fecho de um período. Útil para análise de liquidez e endividamento atual." checked={s.documentos.balancete} onChange={v => update('documentos', { ...s.documentos, balancete: v })} />
                <Check label="IES último ano" tip="Informação Empresarial Simplificada — declaração anual obrigatória que junta dados fiscais, contabilísticos e estatísticos. Submetida até ao 15.º dia do 7.º mês após o fecho do exercício." checked={s.documentos.ies} onChange={v => update('documentos', { ...s.documentos, ies: v })} />
                <Check label="Modelo 22" tip="Declaração anual de IRC. Submetida até 31 de maio do ano seguinte ao exercício. Aplica-se a todas as sociedades, mesmo sem lucro." checked={s.documentos.modelo22} onChange={v => update('documentos', { ...s.documentos, modelo22: v })} />
                <Check label="Declaração IVA" tip="Declarações periódicas de IVA (mensal ou trimestral) — Modelo P. Útil para confirmar enquadramento atual e padrões de IVA suportado/liquidado." checked={s.documentos.dec_iva} onChange={v => update('documentos', { ...s.documentos, dec_iva: v })} />
